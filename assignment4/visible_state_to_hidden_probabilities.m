...
% This takes in the (binary) states of the visible units, and returns the activation probabilities of the hidden units conditional on those states.
    ...
    hidden_input = rbm_w * visible_state;  % size <number of hidden units> by <number of configurations that we're handling in parallel>.
    hidden_probability = 1 ./ (1 + exp(-hidden_input));
end